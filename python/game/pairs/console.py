...
class Console():
    def __init__(self, game_engine):
        ...
        # If we are in console mode
        self.open = False
        self.displayed_height = 0
        self.display_speed = 60 

        #list of strings of all commands written in the command line
        self.history = []
        ...
        self.console_font = pygame.font.SysFont(
                                'Consolas', self.console_font_size)
        self.prompt_text = ">>>"

        self.cursor_char = "|"
        self.cursor_color = (0, 168, 63) #SeaGreen
        #Number of frames for cursor to alternate displaying/not displaying
        self.cursor_blink_cycle = 60
        #Counter variable to time the blink cycle
        self.cursor_blink_timer = 0

        self.backspace_depressed = False
        self.backspace_depressed_timer = 0
        #Number of frames before repeat deletion starts
        self.backspace_repeated_deletion_latency = 30
        #Number of frames each deletion in a repeat deletion takes
        self.backspace_repeated_deletion_speed = 3
 
    def update(self):
        if self.open and self.displayed_height < self.height:
            self.displayed_height = min(self.height, self.displayed_height \
                                                     + self.display_speed)
        if not self.open and self.displayed_height > 0:
            self.displayed_height = max (0, self.displayed_height \
                                            - self.display_speed)
        
        self.cursor_blink_timer = (self.cursor_blink_timer + 1) \
                                  % self.cursor_blink_cycle

        #For backspaces, initial input is handled during the reading of key 
        # events, but repeated deletions are handled in the update function
        if self.backspace_depressed:
            if (self.backspace_depressed_timer \
                >= self.backspace_repeated_deletion_latency) and \
               (self.backspace_depressed_timer \
                % self.backspace_repeated_deletion_speed) == 0:
    ...
    def draw(self, surface):
        if self.displayed_height > 0:
            ...
            surface.blit(cursor_rect,
                         (cursor_width * (self.current_command_cursor \
            ...
            if self.displayed_height == self.height:
                ...
                for i in range(len(self.stdout_history))[::-1]:
                    ...
                    for l in hist_lines:
                        ...
                        line_split = [ l[j:j+chars_per_line] \
                                       for j in range(0, len(l), chars_per_line) ]
                        for line in line_split[::-1]:
                            text_surface = self.console_font.render(line, \
                                                False, self.console_font_color)
    ...
    def get_cursor_color(self):
            ...
            return pairs_utils.color_lerp((255,255,255), self.cursor_color, t)
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key in NUMBERS_SPECIAL_KEYS:
                        if self.caps_on:
                            ...
                            self.current_command_insert_char_at_index(
                                        NUMBERS_SPECIAL_KEYS[event.key])
        ...
        for event in pygame.event.get():
            ...
            if not console.open:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_DOWN:
                        if console.history_index != -1 \
                           and console.history_index < len(console.history) - 1:
                            console.history_index += 1
                            console.current_command = console.history[console.history_index]
                            console.current_command_cursor = len(console.current_command)
                        else:
